...
namespace Framework
{
    ...
    public class SafePool<T> : Pool<T>, ISingleton where T : class, IPoolable, new()
    {
        ...
        #region 单例模式
        protected SafePool() { factory = new DefaultFactory<T>(); }

        public static SafePool<T> Instance
        {
            get { return SingletonProperty<SafePool<T>>.Instance; }
        }
